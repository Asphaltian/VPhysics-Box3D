...
class Box3DPhysicsParseKV final : public IVPhysicsKeyParser
{
public:
	Box3DPhysicsParseKV( KeyValues *pKV );
	~Box3DPhysicsParseKV() override;
...
struct Box3DPhysicsCollisionRulesHelper
{
	ragdollcollisionrules_t	Rules;
	IVPhysicsKeyHandler		*pUnknownKeyHandler;

	Box3DPhysicsIntPair		CollisionPair;
};

//-------------------------------------------------------------------------------------------------

static const Box3DKVSchemaProp_t kSolidDescs[] =
{
...
static const Box3DKVSchemaProp_t kFluidDescs[] =
{
...
static const Box3DKVSchemaProp_t kRagdollDescs[] =
{
...
static const Box3DKVSchemaProp_t kVehicleAxleWheelDescs[] =
{
...
static const Box3DKVSchemaProp_t kVehicleAxleSuspensionDescs[] =
{
...
static const Box3DKVSchemaProp_t kVehicleAxleDescs[] =
{
...
static const Box3DKVSchemaProp_t kVehicleBodyDescs[] =
{
...
static const Box3DKVSchemaProp_t kVehicleEngineBoostDescs[] =
{
...
static const Box3DKVSchemaProp_t kVehicleEngineDescs[] =
{
...
static const Box3DKVSchemaProp_t kVehicleSteeringDescs[] =
{
...
static const Box3DKVSchemaProp_t kVehicleDescs[] =
{
...
static const Box3DKVSchemaProp_t kCollisionRulesDescs[] =
{
...
const char* Box3DPhysicsParseKV::GetCurrentBlockName()
{
	if ( !m_pCurrentBlock )
		return nullptr;
...
bool		Box3DPhysicsParseKV::Finished()
{
	return !m_pCurrentBlock;
}

void		Box3DPhysicsParseKV::ParseSolid( solid_t *pSolid, IVPhysicsKeyHandler *unknownKeyHandler )
{
	...
	ParseBox3DKVSchema( m_pCurrentBlock, kSolidDescs, ARRAYSIZE( kSolidDescs ), pSolid, pSolid, unknownKeyHandler );

	NextBlock();
}

void		Box3DPhysicsParseKV::ParseFluid( fluid_t *pFluid, IVPhysicsKeyHandler *unknownKeyHandler )
{
	if ( unknownKeyHandler )
	...
	ParseBox3DKVSchema( m_pCurrentBlock, kFluidDescs, ARRAYSIZE( kFluidDescs ), pFluid, pFluid, unknownKeyHandler );

	NextBlock();
}

void		Box3DPhysicsParseKV::ParseRagdollConstraint( constraint_ragdollparams_t *pConstraint, IVPhysicsKeyHandler *unknownKeyHandler )
{
	if ( unknownKeyHandler )
	...
	ParseBox3DKVSchema( m_pCurrentBlock, kRagdollDescs, ARRAYSIZE( kRagdollDescs ), pConstraint, pConstraint, unknownKeyHandler );
...
void		Box3DPhysicsParseKV::ParseCustom( void *pCustom, IVPhysicsKeyHandler *unknownKeyHandler )
{
	if ( unknownKeyHandler )
		unknownKeyHandler->SetDefaults( pCustom );

	ParseBox3DKVCustom( m_pCurrentBlock, pCustom, unknownKeyHandler );

	NextBlock();
}

void		Box3DPhysicsParseKV::ParseVehicle( vehicleparams_t *pVehicle, IVPhysicsKeyHandler *unknownKeyHandler )
{
	if ( unknownKeyHandler )
	...
	ParseBox3DKVSchema( m_pCurrentBlock, kVehicleDescs, ARRAYSIZE( kVehicleDescs ), pVehicle, pVehicle, unknownKeyHandler);

	NextBlock();
}

void		Box3DPhysicsParseKV::SkipBlock()
{
	NextBlock();
}

void		Box3DPhysicsParseKV::ParseCollisionRules( ragdollcollisionrules_t *pRules, IVPhysicsKeyHandler *unknownKeyHandler )
{
	if ( unknownKeyHandler )
		unknownKeyHandler->SetDefaults( pRules );

	Box3DPhysicsCollisionRulesHelper helper =
	{
		pRules ? *pRules : ragdollcollisionrules_t{},	// Rules
		unknownKeyHandler,								// pUnknownKeyHandler
	};

	ParseBox3DKVSchema( m_pCurrentBlock, kCollisionRulesDescs, ARRAYSIZE( kCollisionRulesDescs ), &helper, pRules, unknownKeyHandler );
...
void		Box3DPhysicsParseKV::ParseRagdollAnimatedFriction( ragdollanimatedfriction_t *pFriction, IVPhysicsKeyHandler *unknownKeyHandler )
{
	if ( unknownKeyHandler )
	...
	ParseBox3DKVSchema( m_pCurrentBlock, kRagdollAnimatedFrictionDescs, ARRAYSIZE( kRagdollAnimatedFrictionDescs ), pFriction, pFriction, unknownKeyHandler );
...
void		Box3DPhysicsParseKV::NextBlock()
{
	if ( m_pCurrentBlock )
...
{
	VBoxAssertMsg( !bIsPacked, "Packed VPhysics KV not supported. You should not get here anyway as we do not emit it." );
	if ( bIsPacked )
		return nullptr;
	...
	if ( !pszKV )
	{
		Log_Warning( LOG_VBox3D, "CreateVPhysicsKeyParser: Encountered invalid KV data. Falling back to a dummy KV. You may notice a broken prop/vehicle.\n" );

		pszKV = new KeyValues( "VPhysicsKeyParse_Fallback" );
		pszKV->LoadFromBuffer( "VPhysicsKeyParse_Fallback", DummyParserKeyValues );
	}

	return new Box3DPhysicsParseKV( pszKV );
}

void DestroyVPhysicsKeyParser( IVPhysicsKeyParser *pParser )
{
	Box3DPhysicsParseKV *pBox3DParser = static_cast< Box3DPhysicsParseKV * >( pParser );
	delete pBox3DParser;
}
